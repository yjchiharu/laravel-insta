        ...
        @auth
            <div class="col-4 ms-auto mb-3">
                <form action="{{ route('admin.users.search') }}" style="width: 300px;">
                    <input type="search" name="search" id="search" class="form-control form-control-sm " placeholder="Search name...">
                </form>
            </div>
        @endauth
    </div>    
        ...
        <tbody>
            @foreach ($all_users as $user)
                <tr>
                    <td>
                        @if ($user->avatar)
                            <img src="{{ $user->avatar }}" alt="{{ $user->name }}" class="rounded-circle d-block 
                            mx-auto avatar-md">
                        @else
